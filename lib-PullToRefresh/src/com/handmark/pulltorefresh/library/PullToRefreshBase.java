...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
    ...
    protected void onPullToRefresh() {
        switch (mCurrentMode) {
            case PULL_FROM_END:
                Log.e("IUU", "PULL_FROM_END");
                mFooterLayout.pullToRefresh();
                //显示iuu动画，屏蔽自有动画
                mFooterLayout.hideIuuAnimViewWhenStartRefreshing();
                mFooterLayout.needHideBkg(true);
                break;
            case PULL_FROM_START:
                mHeaderLayout.pullToRefresh();
                //显示iuu动画，屏蔽自有动画
                mHeaderLayout.showIuuAnimViewWhenPullRefresh();
                mHeaderLayout.needHideBkg(false);
                break;
            default:
                // NO-OP
                break;
    ...
    protected void onRefreshing(final boolean doScroll) {
        if (mMode.showHeaderLoadingLayout()) {
            mHeaderLayout.hideIuuAnimViewWhenStartRefreshing();
    ...
    private void pullEvent() {
        ...
        if (newScrollValue != 0 && !isRefreshing()) {
            Log.e("IUU", "header height=" + itemDimension);
            float scale = Math.abs(newScrollValue) / (float) itemDimension;
            switch (mCurrentMode) {
                case PULL_FROM_END:
                    mFooterLayout.onPull(scale, true);
                    break;
                case PULL_FROM_START:
                default:
                    mHeaderLayout.onPull(scale, false);
                    break;
            }
